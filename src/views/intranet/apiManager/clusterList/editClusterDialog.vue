  ...
  >
    <div class="create-cluster-dialog__content">
      ...
      <div v-show="active === 3" class="create-success">
        ...
        <p class="create-success__title">集群更新成功</p>
        <p class="update_success_content">
          集群更新内容非即时生效，刷新缓存需要一定时间，99%的缓存刷新时间在1分钟内
        </p>
      </div>
    </div>
    ...
    <div slot="footer" class="create-cluster-dialog__footer">
      ...
      <div v-else-if="active === 3" class="button-wrapper">
        <el-button @click="backApiList">返回API列表</el-button>
        <el-button type="primary" @click="backClusterList"
          >返回集群列表</el-button
        >
      </div>
    </div>
...
export default {
  ...
  methods: {
    async queryCloudClusterDetails() {
      const clusterId = this.id;
      const params = { clusterId };
      cloudCluster
        .queryCloudClusterDetails(params)
        .then((res) => {
          this.clusterBasicInfoData.name = res.data.name;
          this.clusterBasicInfoData.useType = res.data.useType;
          this.clusterBasicInfoData.type = res.data.type;
          this.clusterBasicInfoData.desc = res.data.desc;
          res.data.cloudServerList.forEach((item) => {
            //后端返回数据为boolean类型，转为0和1
            item.resolve ? (item.resolve = 1) : (item.resolve = 0);
            item.isBackup ? (item.isBackup = 1) : (item.isBackup = 0);
            item.down ? (item.down = 1) : (item.down = 0);
          });
          this.addIpPortData.cloudServerList = res.data.cloudServerList;
          this.setTheTimeoutData.connectTimeout = res.data.connectTimeout;
          this.setTheTimeoutData.readTimeout = res.data.readTimeout;
          this.setTheTimeoutData.sendTimeout = res.data.sendTimeout;
          this.setTheTimeoutData.reliveTimeout = res.data.reliveTimeout;
        })
        .catch((error) => {
          console.log(error);
          this.loading = false;
    ...
    async cloudClusterUpdate() {
      ...
      return await basicInfo.cloudClusterUpdate(data).catch((error) => {
    ...
    backClusterList() {
      ...
      this.$emit("update:editClusterDialogVisible", false);
      this.$emit("queryRefresh", true);
    },
    backApiList() {
      this.$emit("update:editClusterDialogVisible", false);
      this.$emit("tabsValue", true);
    },
    dialogBeforeClose() {
      const formList = [
        "clusterBasicInfoData",
      ...
      this.$refs[hashMap[this.active]].validate(async (valid) => {
        if (valid) {
          if (this.active === 2) {
            ...
            await this.cloudClusterUpdate().catch((error) => {
  ...
  mounted() {
    if (this.editClusterDialogVisible) {
      this.queryCloudClusterDetails();
    }
  },
};
</script>
