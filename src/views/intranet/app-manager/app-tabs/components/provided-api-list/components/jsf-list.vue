      ...
      >
        <template slot-scope="scope">
          <a
            ...
            v-on:click="onGotoApiOverView(scope.row.id)"
            class="link"
...
export default {
  ...
  created() {
    this.queryApiOfProvider(1, this.listQuery.limit);
  },
  methods: {
    ...
    queryApiOfProvider() {
      gwApiRequest({
        method: "post",
        url: "/ApiManagerService/queryApiInApp",
        data: JSON.stringify([{
          appKey: this.appKey
    ...
    paginationChange(p) {
      ...
      this.queryApiOfProvider();
    },
    onGotoApiOverView(id) {
      this.$router.push({
        name: "ApiManagerDetail",
        params: { id: id, type:"jsf"},
