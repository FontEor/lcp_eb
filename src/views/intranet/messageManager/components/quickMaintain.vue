  ...
  <div class="quick-maintain">
    ...
    >
      ...
      <el-dropdown-menu slot="dropdown">
        <el-dropdown-item
          v-if="importTypes.includes('importXml')"
          command="importXml"
          >导入XML文件</el-dropdown-item
        >
        <el-dropdown-item
          v-if="importTypes.includes('importJson')"
          command="importJson"
          >导入JSON报文</el-dropdown-item
        >
        <el-dropdown-item
          v-if="provideWay === 1 && importTypes.includes('pullFromJsf')"
          command="pullFromJsf"
    ...
    >
      ...
      <div slot="footer">
        ...
        <el-button
          type="primary"
          @click="handleSaveImportJson"
          :disabled="saveJSONing"
          :clstag="clstag('messageManager_1669964723128|2', '')"
          >保存</el-button
        >
      </div>
    </el-dialog>
...
export default {
  ...
  props: {
    ...
    importTypes: {
      type: Array,
      default: () => ["importXml", "importJson", "pullFromJsf"]
    },
    pQuickMaintainDocByJson: {
      type: Function
    }
  },
  data() {
    return {
      ...
      importJsonFormRules: {
        method: [{ required: true, message: "请选择导入方式" }],
        fileList: [{ type: "array", required: true, message: "请上传JSON文件" }]
      },
      importJsonDialogVisible: false,
      computeQuickMaintainTipText: "更新将覆盖原有消息体参数，请谨慎操作！",
      saveJSONing: false
    };
  },
  methods: {
    ...
    async findApiStatus() {
      ...
      if (!qualifiedName || !methodName || !alias) {
        this.$message.error(
          "从注册中心拉取不到元数据，请确认在【基础数据】维护的信息是否正确？"
        );
    ...
    handleClickPullFromJsf() {
      ...
      this.pullFromJsfDebounceTimer = setTimeout(async () => {
        ...
        this.handleUpdateParamList(paramList, this.isRequest);
    ...
    handleSaveImportJson() {
      this.$refs.importJsonForm.validate(async valid => {
        if (valid) {
          ...
          this.$message.success("导入成功，可继续在页面维护文档详细信息");
          this.saveJSONing = false;
        } else {
          return false;
        }
    ...
    async quickMaintainApiDocByJson(jssKey) {
      ...
      const data = [{ jssKey, topic }];
      if (this.pQuickMaintainDocByJson) {
        return await this.pQuickMaintainDocByJson(data);
      }
    }
  }
};
